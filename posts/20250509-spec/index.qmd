...
Its proof is relatively straightforward and is omitted.
...
- $\mathbb F_p[x]$-identified prime ideals (where $p$ is a prime number):
  - $(p)$
...
- $K(x)[y]$-identified prime ideals:
  - The zero ideal $(0)$
  - $(f)$ where $f \in K[x,y]$ is an irreducible polynomial of degree $\geq 1$
- $K[x,y] / (p(x))$-identified prime ideals (where $p(x)$ is an irreducible polynomial of $K[x]$):
  - $(p)$
