...
For the backward direction, it suffices to assume $n$ is square-free. Say its decomposition is $n = \prod_{i=1}^k p_k$ where $p_i = 2$ or $p_i \equiv 1 \pmod 4$. By [@cor-prime-two-squares], $p_i = \N(z_i)$ for some $z_i \in \mathbb Z[\i]$. Thus $n = \prod_{i=1}^k p_k = \N \left( \prod_{i=1} z_i \right)$, thus a sum of two squares.
:::
