...
- 可以参考 [@neukirch_algebraic_1999, {chapter 1, proposition 2.2}] 的证明，是证明 integral closure 构成环的一个中间步骤．因为代数数论的世界比较美好（例如线性变换都是乘一个扩环里的数，自动交换），写出来会好看一些．
...
再稍微推广一点的版本要求 $\varphi (M) \subset \mathfrak a M$，这里 $\mathfrak a$ 是 $A$ 的一个理想．这时 $\Phi$ 的全体内容落在 $\mathfrak a$ 中，于是取得零化多项式的全体系数也落在 $\mathfrak a$ 内（见 [@m_f_atiyah_introduction_1969, {proposition 2.4}]）．此命题用于证明著名的 Nayakama 引理．