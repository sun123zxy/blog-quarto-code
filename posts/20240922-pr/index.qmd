...
$$
Z_{p^\alpha}^\times \cong \left( 1 + p Z_{p^\alpha}^\times \right) \times Z_p^\times
$$
...
Hensel 引理的递归构造其实暗藏更多结构．我们将一点直觉在下图中展示，感兴趣的读者可参见 [@gupta_padic_2018, {section 6--7}]．
...
Z_{p^\alpha}^\times \cong \left( 1 + p Z_{p^\alpha}^\times \right) \times Z_p^\times
$$
接下来的工作是分别研究 $1 + p Z_{p^\alpha}^\times$ 和 $Z_p^\times$．
...
## $1 + p Z_{p^\alpha}^\times$ 的结构：二项式定理与含 $p$ 量估计 {#sec-nup}

这里处理 $1 + p Z_{p^\alpha}^\times$ 的结构的办法相当暴力 [@dummit_abstract_2003, {section 2.4, exercises 21--23, p. 60}]：
...
设 $p$ 是奇质数，$\alpha \ge 1$ 是正整数，则元素 $1+p \in 1 + p Z_{p^\alpha}^\times \le Z_{p^\alpha}^\times$ 的阶为 $p^{\alpha-1}$．
:::
...
设 $p$ 是奇质数，$\alpha \ge 1$ 是正整数，则 $1 + p Z_{p^\alpha}^\times \cong Z_{p^{\alpha-1}}$．
:::

:::{#prp-kerphi-evenp}

设正整数 $\alpha \ge 2$．元素 $5 = 1+ 2^2 \in 1 + p Z_{p^\alpha}^\times = Z_{2^\alpha}^\times$ 的阶为 $2^{\alpha-2}$．
:::
...
设 $\alpha \ge 2$ 是正整数，则元素 $-1$ 和 $1+2^{\alpha-1}$ 在 $1 + p Z_{p^\alpha}^\times = Z_{2^\alpha}^\times$ 的阶为 $2$．当 $\alpha \ge 3$ 时两者互异．
:::
...
设 $\alpha \ge 2$ 是正整数，则 $Z_{2^\alpha}^\times = 1 + p Z_{p^\alpha}^\times \cong Z_2 \times Z_{2^{\alpha-2}}$．
:::

:::{.proof}

当 $\alpha = 2$，$|1 + p Z_{p^\alpha}^\times| = 2$，平凡．当 $\alpha \ge 3$，[@prp-kerphi-notcyc] 给出了两个无交且生成整个 $1 + p Z_{p^\alpha}^\times$ 的循环群，使用子群直积的识别定理即可．
:::
