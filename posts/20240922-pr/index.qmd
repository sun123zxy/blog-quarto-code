...
对 $F$ 的要求甚至还可进一步减弱为整环 $R$——这是因为带余除一个首一多项式并不需要可逆元的参与，亦可通过取 $R$ 的分式域直接证明．进一步分析，$R$ 的特定阶数有限乘法子群要么不存在，要么唯一且是循环群，并按阶数的整除关系相互构成包含关系．这些有限乘法子群的并便是 $R$ 的乘法群的挠子群（torsion subgroup）．
:::
...
设 $\alpha \ge 2$ 是正整数，则 $Z_{2^\alpha}^\times = \ker \pi \cong Z_2 \times Z_{2^{\alpha-2}}$．
:::
...
### Fermat 骗子群，Carmichael 函数与 Carmichael 数
...
$$
\lambda(m) := \lcm_{a \in Z_m^\times} \ord(a)
$$
