...
因为 $Z_2^\times$ 是平凡群，故只需研究 $p \ge 3$ 的情况．事实上我们已经比较清楚环 $Z_p$ 的结构——它是一个域．[@dummit_abstract_2003, {section 9.5, pp. 313--314}] 给出了一种利用域上多项式环性质证明 $Z_p^\times$ 是循环群的神奇做法．我们知道，域上的 $n$ 阶多项式至多只能有 $n$ 个根（数论中又名 Lagrange 定理），而 $Z_p^\times$ 内满足 $x^d = 1$ 的元素（即 $d$ 阶单位根）都是多项式 $x^d - 1$ 的根，故 $d$ 阶单位根的个数至多为 $d$ 个．令人惊奇的是，如此松弛的估计足以确定 $Z_p^\times$ 的结构．请看如下定理 [@dummit_abstract_2003, {section 6.1, proposition 5, p. 192}]．
...
设 $n$ 次单位根群 $U_n := \{ x \in R: x^n = 1 \} \leq \Tor R^\times \leq R^\times$，它们的并构成了整环 $R$ 的乘法群 $R^\times$ 的挠子群（torsion subgroup）$\Tor R^\times$．我们的推导实际上说明：

- 每个 $U_n$ 都是循环群，也是乘法群 $R^\times$ 唯一的 $n$ 阶子群．

- 若 $d | n$，则 $U_d \leq U_n$．特别地，阶数互素的两群之交为单位元群．

- 使用 $\mathbb Z$-模风格的记号，
  $$
  \Tor R^\times = \bigoplus_{p \in \text{prime}} \bigcup{\alpha=1}^{+\infty} U_{p^\alpha}
  $$
