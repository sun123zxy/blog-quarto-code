...
不妨设 $c = \sum_{g \in \mathcal S_n} n_g g$ 是满足对任意 $p \in P_T$, $q \in Q_T$，有 $p c q = \sgn(q) c$ 的某一 $\mathbb C[\mathcal S_n]$ 中元素．注意我们已经知道满足这条件的 Young 对称化子 $c_T \in P_T Q_T$，所以我们应当指望，也只需证明如下两个事实：
...
可设 $c_T^2 = \alpha c_T$，$\alpha \in \mathbb C$．这意味着 $c_T$ 作为左乘在 $\mathbb C[\mathcal S_n]$ 上的线性变换几乎是幂等变换，此时 $\ker c_T \oplus \im c_T = \mathbb C[\mathcal S_n]$．这里 $\im c_T$ 就是 $V_T$，因此 $c_T$ 的迹 $\tr c_T = \alpha \dim V_T$．又注意到 $c_T$ 作用在 $g$ 上得到向量的 $g$ 分量对应系数是 $1$，故 $\tr c_T = n!$．综上 $\alpha = n! / \dim V_T$．
:::
...
注意直到该定理前，我们都没有用到 $\mathbb C$ 的任何性质．在此定理中我们使用了 $\mathbb C$ 特征为 $0$ 导致的群代数 $\mathbb C[\mathcal S_n]$ 的半单性，故 Specht 模的不可约性对任意特征为 $0$ 的域都成立．
:::
