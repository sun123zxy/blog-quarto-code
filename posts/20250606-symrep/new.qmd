...
设 $T, S$ 是形状为 $\lambda$ 的两个 Young 表，则以下条件等价：

- 任意一对 $T$ 中同行的数字均落在 $S$ 中的不同列
- 存在 $p \in P_T,q' \in Q_S$ 使得 $pT = q'S$
- 存在 $p \in P_T$, $q \in Q_T$ 使得 $S = pqT$
:::
...
$c_T x c_T \in \mathbb C c_T$ 对任意 $x \in \mathbb C[\mathcal S_n]$ 成立．特别地，$c_T^2 \in \mathbb C c_T$．
:::

:::{#cor-cxc-dim}

对同形状 Young 表 $T$, $S$，$\dim c_S \mathbb C[\mathcal S_n] c_T = 1$．
:::

:::{.proof}

设 $S = gT$，则 $c_S \mathbb C[\mathcal S_n] c_T = g c_T \mathbb C[\mathcal S_n] c_T$，因此只需考察 $c_T \mathbb C[\mathcal S_n] c_T$ 的维数．由 [@cor-cxc-dim]，只需证其中存在非零元．直接用 $V_T \to V_T$ 的恒等映射即可断言确有非零元．
:::

## 不同形状的情形

受之前 [@prp-srdc] 刻画同形状 Young 表间群作用关系的启发，我们可能也需要从组合上对 Young 表形状不同一事做一点文章．以后设 $T,S$ 为形状分别为 $\lambda, \mu \vdash n$ 的 Young 表，不妨设 $\lambda > \mu$（这里全序取为 $n$ 的整数划分的字典序）．
...
对不同形状 Young 表 $T$, $S$，$\dim c_S \mathbb C[\mathcal S_n] c_T = 0$．
:::
