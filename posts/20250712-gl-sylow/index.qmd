...
这个数字 $[n]_q!$ 同时也是 comlete flag variety 的大小，因为完全旗簇和 $G/B$ 之间有一一对应．此事有 $q$-analog 风味的对应物：在 $\{1,\dots,n\}$ 上的“完全旗簇”的数量是 $n!$．

$[n]_q$ 是 $\mathbb F_q$ 上 $n-1$ 维投射空间的大小，正如 $n$ 是 $\{1,\dots,n\}$ 的大小．

回忆 Sylow 第一定理的一个证明：将群嵌入 $S_n$ 再嵌入 $\GL_n(\mathbb F_p)$，取上三角找到 Sylow $p$-子群共轭取交拉回原群．
:::
...
## $[B_n,B_n] = U_n$？$[U_n,U_n] = U_{n-1}$？

注意 $U_n$ 可由全体 $1 + y E_{i,j}$ 生成，这里 $E_{i,j}$ 是基本矩阵，$i < j$, $y \in F$．使用对角矩阵和 $1 + E_{i,j}$ 做交换子即可凑出任意 $1 + y E_{i,j}$．故 $[B_n,B_n] = U_n$．
