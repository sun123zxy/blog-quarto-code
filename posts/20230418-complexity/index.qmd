...
故用 @prp-trick3ex ，当 $K > \sqrt n$ 时，算法在递归部分的时间复杂度降低为
...
这估计并不算优秀. 传言存在 $O(n^{\frac 2 5})$ 的估计，猜测大概优化了 $\left\{ \floor{\frac n i} \right\}_{i=1}^n$ 和 $\left\{ \floor{ \left( \floor{\frac n d} \right)^{\frac 1 2} }\right\}_{d \in D_2 (n)}$ 的重叠部分。笔者尚未找出其推导方式.

upd: 关于 $O(n^{\frac 2 5})$ 的估计，请参阅博客[@nonsquare_blog]和论文[@nonsquare_paper]．