...
设 $V$ 是域 $K$ 上的有限维（逆变的）线性空间，$V^*$ 是它的（协变的）对偶空间，$\langle -,- \rangle_{\vec g}$ 是配备在 $V$ 上的一个非退化对称双线性型．回忆泛性质
$$
\rm{Hom}(V, \rm{Hom}(V, K)) = \rm{Hom}(V, V^*) \cong V^* \otimes V^*
$$
故我们可以把 $\vec g$ 看作 $V \to V^*$ 的线性映射，或吃入两个 $V$ 中向量、吐出标量 $K$ 的张量 $\vec g \in V^* \otimes V^*$．以后我们往往使用如下记号表达这一事实：
$$
\langle \vec a, \vec b \rangle_{\vec g} = (\vec g (\vec a)) (\vec b) = g_{\alpha \beta} a^\alpha b^\beta
$$
这里后者是张量缩并的*抽象指标记号（abstract index notation）*：$\alpha$ 和 $\beta$ 作为抽象指标，在缩并运算前后上下配对，从而标记了以何种顺序将向量 $\vec a \in V$, $\vec b \in V$ 自然地填入张量 $g \in V^* \otimes V^*$．

使用 $g$，我们得以将向量在 $V$ 和 $V^*$ 间相容地转移．更有趣的是，在抽象指标记号下，我们事实上可以不再区分某一向量 $\vec a$ 具体属于 $V$ 还是 $V^*$：当需要表达逆变的 $\vec a \in V$ 时，我们总将其写为 $a^\alpha \in V_\alpha$；当需要其对应的协变向量 $\vec g (\vec a) \in V^*$ 时，我们直接记其为 $a_\beta = g_\alpha a^\alpha \in V^*_\beta$．注意这里我们也在空间 $V$ 和 $V^*$ 加上下标以标记其所属空间，类似的可将 $\vec g \in V^* \otimes V^*$ 写作 $g_{\alpha \beta} \in V^*_\alpha \otimes V^*_\beta$．

有时我们也需要将协变向量 $a_\beta \in V^*$ 搬回 $V$．已经有了 $V \to V^*$ 的 $g$，自然使用它的逆作为拉回．我们将其写作 $g^{\alpha \beta} \in V^\alpha \otimes V^\beta \cong \rm{Hom}(V^*_\alpha, V_\beta)$，则 $a^{\alpha} = g^{\alpha \beta} a_{\beta}$（由于 $\vec g$ 的对称性，我们可以不区分 $\alpha$ 和 $\beta$ 的顺序）．类似的还可定义 $g^\alpha_\beta \in V^\alpha \otimes V^*_\beta \cong \rm{Hom}(V_\beta, V_\alpha) \cong \rm{Hom}(V_\alpha^*, V_\beta^*)$，它事实上是恒同映射 $\mathbf{id}_V$ 或 $\mathbf{id}_{V^*}$ 的张量表达，在作用在向量 $a^\alpha$ 上时，仅起到更换指标记号的作用：$a^\beta = g_{\alpha}^{\beta} a^\alpha$．鉴于其与双线性型 $\vec g$ 事实上无关，我们往往也将省略 $g^\alpha_\beta$ 写作统一的 $\delta^\alpha_\beta$．在后面将会提到的矩阵形态中，它对应着 Kronecker 记号．

## 具体张量

接下来，我们考虑在选定基底后，抽象张量在坐标下的表示．什么是表示？表示是将抽象线性空间 $V$ 映到坐标空间 $K^{\oplus n}$ 的线性映射．在坐标空间有标准的内积 $\langle \vec x, \vec y \rangle = \sum_{i=1}^n x^i y^i$ 可供使用，我们希望经过该表示映射两侧的内积结构相容．
