...
设 $V$ 是域 $K$ 上的有限维（逆变的）线性空间，$V^*$ 是它的（协变的）对偶空间．自然地，一个 $V^*$ 中的协变向量 $\vec b$ 作用在 $V$ 中的逆变向量 $\vec a$ 上，将获得标量 $\vec b(\vec a) \in K$．这种操作被称为缩并运算，在单个线性空间上并不复杂．但当我们考虑一堆线性空间及其对偶空间的混合张量积时，事情变得复杂起来．例如，考虑 $\vec a \in V \otimes V \otimes V^*$，$\vec b \in V \otimes V^* \otimes V^*$，若我们希望通过将 $\vec a$ 第一个分量喂入 $\vec b$ 的第三个分量，其它分量不做缩并仅按张量积顺序排列得到新张量 $\vec c \in V \otimes V^* \otimes V \otimes V^*$，什么样的记号体系可以帮助我们清晰地表达这一过程？
...
构造表示的本质是选定基底．选定基底 $(\vec e_i)_{1 \leq i \leq n} \subseteq V$，这就规定了同构 $\vec e : K^{\oplus n} \to V$．我们大可继续利用张量积泛性质将此同构看作混合张量 $e_i^\alpha \in \left( K^{\oplus n} \right)^*_i \otimes V^\alpha$．这里，$i$ 被视为标记坐标空间 $K^{\oplus n}$ 的指标，我们称之为*坐标指标*，以区别于用于标记 $V$ 的抽象指标 $\alpha, \beta, \dots$．
...
综上我们看到，形如 $e \cdot - \cdot e$ 的张量坐标表示完成了从抽象混合张量到坐标混合张量的保缩并同构．形式上，这是把混合张量的抽象指标 $\alpha, \beta, \dots$ 替换为坐标指标 $i, j,\dots$ 等的过程．我们以后把这一同构记作 $\mathcal M$．
