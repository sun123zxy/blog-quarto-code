  ...
  html:
    other-links:
      - text: "在知乎上阅读"
        ...
        href: "https://zhuanlan.zhihu.com/p/2010361438884106334"
---
...
本文中，我们尝试改装出一套代数批友好的 Einstein 求和记号体系．这套语言是纯代数的：仅依赖对偶空间和张量积的泛性质，并使 Einstein 求和、指标升降的记号获得更自然的解释．注意笔者没有物理 / 微分几何背景，和标准处理手段可能会有出入，欢迎指正．
...
$$
类似地，$b^{\delta}_{\epsilon \beta} \in V^\delta \otimes V^*_\epsilon \otimes V^*_\beta$ 在基底 $(\vec e_i)$ 和对偶基底 $(\vec e^i)$ 下的坐标表示为
$$
...
Python 的 numpy / torch 库均有 $\mathbb R^n$ 标准内积下的 Einstein 求和实现．
...
由于我们在标准内积下工作，故可以无视协变反变 / 上下标的区别．`ik,kj->ij` 是 $c_i^j = a_i^k b_k^j$ 的更紧凑写法．上述代码实现了矩阵乘法．也可以直接使用 `np.dot` 或 `np.matmul` 完成．一般来说，`np.dot(a, b)` 会缩并 `a` 最后一个指标（`k`）和倒数第二个指标（`k`）．但直接使用 Einstein 求和约定显然降低了背诵 API 的负担．

~~是不是和你以为的使用例不太一样 :p~~，这里暂时标个 TODO，将来有机会再补充这套记号搬到物理 / 微分几何背景下的使用例．
