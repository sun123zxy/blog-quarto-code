...
研究该定理 $a+b = p^m$ 的特例．用下降阶乘幂形式考虑二形式系数中 $k$ 递增的过程，注意到 $\nu_p (p^m-k) = \nu_p(k)$，可以发现分子新增的 $n-k+1$ 和分母新增的 $k$ 恰好错一位，于是
$$
...
\end{cases}
$$
其中 $p$ 是某一质数．这符合 Kummer 定理的直观描述．得到 $p^m$ 的加法必然会出现进位，在加法 $p^{m-1} + p^{m-1}$ 时达到最小进位次数 $1$；否则在任一质数进制 $p$ 下，都可以取加法 $\operatorname{lowbit}_p (n) + (n - \operatorname{lowbit}_p (n))$ 使得不发生进位．
