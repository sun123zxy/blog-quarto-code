...
|:-:|:-:|:-:|:--:|:--:|:--:|
| 有标号 | 有标号 	| 允许	 | $$r^n$$ | | |
| 有标号 | 有标号 	| 不允许 | $$r! {n \brace k}$$ | $$\mathrm{EGF}(x)=(e^x - 1)^r$$ | |
...
| 有标号 | 不限制 	|       | $$\sum_{k=0}^n {n \brace k}$$ | $$\mathrm{EGF}(x)=e^{e^x - 1}$$ | Bell 数 |
| 无标号 | 有标号 	| 允许	 | $$\binom{n+r-1}{r-1}$$ | | $r$ 元不定方程非负整数解 |
| 无标号 | 有标号 	| 不允许 | $$\binom{n-1}{r-1}$$ | | $r$ 元不定方程正整数解 |
