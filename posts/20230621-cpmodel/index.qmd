...
| 代号 | $n$ 个球 | $r$ 个盒 | 空盒否 | 递推或封闭形式 | 生成函数 | 备注 |
|:-:|:-:|:-:|:-:|:--:|:--:|:--:|
| $${1,1,0}$$ | 有标号 | 有标号 	| 盒可空 | $$r^n$$ | | $[n] \to [r]$ 映射个数 |
| $${1,1,1}$$ | 有标号 | 有标号 	| 盒非空 | $$r! {n \brace r} = \sum_{k=0}^r (-1)^{r-k} \binom r k k^n$$ | $$\begin{aligned} \mathrm{EGF}_r(x)&=(e^x - 1)^r \\ \mathrm{EGF}_n(x) &= e^{-x} \sum_{k=0}^{+\infty} k^n \frac{x^k}{k!} \end{aligned}$$ | $[n] \to [r]$ 满射个数 |
| $${1,0,0}$$ | 有标号 | 无标号 	| 盒可空	 | $$\sum_{k=0}^r {n \brace k}$$ | $$\mathrm{EGF}_r(x) = \sum_{k=0}^r \frac {(e^x - 1)^k}{k!}$$ | |
| $${1,0,1}$$ | 有标号 | 无标号 	| 盒非空 | $$\begin{aligned} {n \brace r} &= \sum_{k=0}^r \frac{k^n}{k!} \cdot \frac{(-1)^{r-k}}{(r-k)!} \\ {n+1 \brace r} &= r {n \brace r} + {n \brace r-1} \end{aligned}$$ | $$\begin{aligned} \mathrm{EGF}_r(x) &= \frac{(e^x - 1)^r}{r!} \\ \mathrm{OGF}_n(x) &= e^{-x} \sum_{k=0}^{+\infty} k^n \frac{x^k}{k!} \end{aligned}$$ | 第二类 Stirling 数 |
| $${1,\emptyset,\emptyset}$$ | 有标号 | 不限制 	|       | $$\begin{aligned} B(n) &= \sum_{k=0}^n {n \brace k} \\ B(n+1) &= \sum_{k=0}^n \binom n k B(k) \end{aligned}$$ | $$\mathrm{EGF}(x)=e^{e^x - 1}$$ | Bell 数；$[n]$ 上等价关系（划分）个数 |
| $${0,1,0}$$ | 无标号 | 有标号 	| 盒可空 | $$\binom{n+r-1}{r-1}$$ | $$\mathrm{OGF_r}(x) = \left(\frac 1 {1-x}\right)^r$$ | $r$ 元不定方程非负整数解个数 |
| $${0,1,1}$$ | 无标号 | 有标号 	| 盒非空 | $$\binom{n-1}{r-1}$$ | $$\begin{aligned} \mathrm{OGF_r}(x) &= \left(\frac x {1-x}\right)^r \\ &= x^r \left(\frac 1 {1-x}\right)^r \end{aligned}$$ | $r$ 元 $n$ 次单项式构型数 |
| $${0,0,0}$$ | 无标号 | 无标号 	| 盒可空 | $$\sum_{k=0}^r p(n,k)$$ | $$\mathrm{OGF}(x)= \frac 1 {1-x} \prod_{k=1}^{+\infty} \frac 1 {1-x^k}$$ | |
| $${0,0,1}$$ | 无标号 | 无标号 	| 盒非空 | $$p(n,r)=p(n-1,r-1)+p(n-r,r)$$ | | $r$ 部分拆数 |
| $${0,\emptyset,\emptyset}$$ | 无标号 | 不限制	|       | $$\begin{aligned} p(n) &= \sum_{\substack{k \in \mathbb Z}} (-1)^k p_{n-k(3k-1)/2} \\ &= p_{n-1} + p_{n-2} - p_{n-5} - p_{n-7} + \dots \end{aligned}$$ | $$\begin{aligned} \mathrm{OGF}(x)&=\prod_{k=1}^{+\infty} \frac 1 {1-x^k} \\ &= \left( \sum_{k \in \mathbb Z} (-1)^k x^{k(3k-1)/2} \right)^{-1} \end{aligned}$$ | 分拆数 |
:::
...
#### 第二类 Striling 数同列计算：$\mathrm{EGF}_r^{1,0,1}(x)$，$\mathrm{EGF}_r^{1,1,0}(x)$ 与 $\mathrm{EGF}^{1,\emptyset,\emptyset}(x)$
...
Bell 数 $B(n)$ 的组合定义即 $1,\emptyset,\emptyset$-球盒问题的方案数 $\mathrm{BB}_{1,\emptyset,\emptyset}$，亦可描述为 $n$ 元集合上等价关系（划分）的数量．

注意到 $1,0,1$-球盒问题和 $1,\emptyset,\emptyset$-球盒问题的关系
$$
\mathrm{BB}_{1,\emptyset,\emptyset}(n) = \sum_{r=0}^{n} \mathrm{BB}_{1,0,1}(n,r) = \sum_{r=0}^{+\infty} \mathrm{BB}_{1,0,1}(n,r)
$$
我们首先有
$$
...
$$
\mathrm{EGF}^{1,\emptyset,\emptyset}(x) = \sum_{r=0}^{+\infty} \mathrm{EGF}_r^{1,0,1}(x) = \sum_{r=0}^{+\infty} \frac{(e^x - 1)^r}{r!} = e^{e^x - 1}
$$
...
问得好，但这是个著名的 open problem．各种类型的偏序图计数参考 [Partially ordered set - Wikipedia # Number of partial orders](https://en.wikipedia.org/wiki/Partially_ordered_set#Number_of_partial_orders)．我们再给出若干参考资料：

-   <https://math.stackexchange.com/questions/2677986/how-many-partial-orders-can-be-defined-on-varnothing-and-a-finite-set>
...
```c++
// polynomial universal, 2023/07/13
...
Poly operator << (Poly A,ll shift){ // optional
...
Poly operator >> (Poly A,ll shift){ // optional
	ll n=A.len()-shift; if(n<=0) return {0};
...
Poly itg(Poly A,ll c){ // integral
	A.push_back(0); for(ll i=A.len()-1;i>=1;i--) A[i]=A[i-1]*inv(i)_;
	A[0]=c; return A;
}
```
...
class Poly : public vector<ll>{ public:
	// ... (Poly defaults)
	void DFT(ll typ){ // to be called by opt() only (with proper length and rev array)
		ll n=len();
		...
		for(ll hf=1;hf<n;hf*=2){
			...
			for(ll i=0;i<n;i+=hf*2){
				...
				for(ll k=0;k<hf;k++){
					...
					at(i+k)=lmodv(x+y); at(i+hf+k)=lmodv(x-y);
					wk=wk*w _;
				}
			}
		}
...
// --- optimized ---
Poly opt(vector<Poly> pv,ll len,std::function<ll(const vector<ll>&)> func){
	ll n=ceil(log2(len)); spawn_rev(n); n=1<<n;
	for(auto &P : pv) P.resize(n),P.DFT(1);
	Poly C(n); for(ll i=0;i<n;i++){
		vector<ll> lv; for(auto &P : pv) lv.push_back(P[i]);
		C[i]=func(lv);
	}
	C.DFT(-1); C.resize(len);
	return C;
}
Poly operator * (Poly A,Poly B){
	return opt({A,B},A.len()+B.len()-1,
		[](const vector<ll> &lv)->ll{return lv[0]*lv[1]_;}
	);
}
```
...
// optimized
Poly inv(const Poly &A){
	ll n=A.len(); Poly B={inv(A[0])};
	for(ll hf=1;hf<n;hf<<=1){
		B=opt({A.subpoly(0,hf*2),B},hf*4,[](const vector<ll> &lv)->ll{
			ll a=lv[0],b=lv[1]; return hmodv((2-a*b)_*b);
		}); B.resize(hf*2);
	} B.resize(n); return B;
}
```
...
Poly sqrt(const Poly &A){
	ll n=A.len(); Poly B={1}; // assume a_0 = 1 and b_0 positive
	for(ll hf=1;hf<n;hf<<=1){
...
```c++
Poly qpow(const Poly &A,ll up){return exp(ln(A)*up);} // assume a_0 = 1
```
...
```c++
// Martix universal 2023/07/16
class Matrix : public vector<ll>{public:
	static Matrix ID(ll n){
		Matrix A(n,n); for(ll i=0;i<n;i++) A(i,i)=1;
		return A;
	}
	ll row,col;
	template<typename... Args>
	Matrix(ll row,ll col,Args... args) : row{row},col{col},vector<ll>(row*col,args...){}
	Matrix(ll row,ll col,initializer_list<ll> lst) : row{row},col{col},vector<ll>(lst){}
	// Matrix(const Matrix &other) = default;
	ll& at(ll i,ll j){return vector<ll>::at(i*col+j);}
	ll cat(ll i,ll j) const {return *(begin()+i*col+j);}
	ll& operator () (ll i,ll j){return at(i,j);}
	Matrix submat(list<ll> rlst,list<ll> clst){
		Matrix A(rlst.size(),clst.size(),{});
		for(ll r : rlst) for(ll c : clst) A.push_back(at(r,c));
		return A;
	}
	Matrix remmat(unordered_set<ll> rst,unordered_set<ll> cst){
		Matrix A(row-rst.size(),col-cst.size(),{});
		for(ll r=0;r<row;r++) if(!rst.count(r))
			for(ll c=0;c<col;c++) if(!cst.count(c))
				A.push_back(at(r,c));
		return A;
	}
	void debug(){
		cout<<'('<<row<<','<<col<<')'<<'\n';
		for(ll i=0;i<row;i++){for(ll j=0;j<col;j++) cout<<at(i,j)<<',';cout<<'\n';}
	}
};
int main(){
	Matrix E={2,3,{1,2,3,
				   4,5,6}},F(2,3,3),G(F);
	E.debug(); F.debug();G.debug();E.remmat({},{0,2}).debug();
}
```
	...
	for(ll i=0;i<A.row;i++)
		for(ll j=0;j<B.col;j++)
			for(ll k=0;k<A.col;k++)
				mmodp(C(i,j)+=A.cat(i,k)*B.cat(k,j));
	return C;
}
```
...
```c++
Matrix qpow(Matrix X,ll up){
	Matrix A=Matrix::ID(X.row); if(X.row!=X.col) return Matrix(0,0);
	while(up){
		if(up&1) A=A*X;
		X=X*X; up>>=1;
	}return A;
...
下面给出的实现在消去时使用辗转相除法，可用于任意 Euclid 整环（Euclidean domain，有带余除法的无零因子的交换幺环）上的模（环上的线性空间），且时间复杂度不会增加．一个常见的应用是模 $m$ 整数环 $\mathbb Z_m$ 上的行列式求值，其中 $m$ 不是质数．
...
```c++
ll det(Matrix A){
	ll n=A.row; if(A.row!=A.col) return 0;
	ll ans=1;
	for(ll c=0;c<n;c++){
		auto swpl=[&](ll a,ll b){
			lmodp(ans*=-1); for(ll k=c;k<n;k++) swap(A(a,k),A(b,k));
		};
		for(ll r=c+1;r<n;r++){
			while(A(c,c)!=0){
				ll factor=A(r,c)/A(c,c);
				for(ll k=c;k<n;k++) hmodp(A(r,k)-=A(c,k)*factor);
				swpl(c,r);
			}swpl(c,r);
		}
		/*ll r; for(r=c;r<n;r++) if(A(r,c)!=0) break;
		if(r==n) return 0;
		if(r!=c) swpl(r,c);
		for(r=c+1;r<n;r++){
			ll factor=inv(A(c,c))*A(r,c)_;
			for(ll k=0;k<n;k++) hmodp(A(r,k)-=A(c,k)*factor);
		}*/
	}
	for(ll c=0;c<n;c++) mmodp(ans*=A(c,c));
	return ans;
}
```
