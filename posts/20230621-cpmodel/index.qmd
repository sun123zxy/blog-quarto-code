...
\end{aligned}
$$
其中 $A$ 代表全集，$A_i$ 代表满足第 $i$ 个性质的元素构成的集合，$i \in U$．
:::
...
\end{gathered}
$$
取 $S=\varnothing$ 代入 [@thm-inex-first-set] 右侧就得到结论．
:::

:::{#cor-inex-first-ordinary}
...
| $${0,1,0}$$ | 无标号 | 有标号 	| 盒可空 | $$\binom{n+r-1}{r-1}$$ | | $r$ 元不定方程非负整数解个数 |
| $${0,1,1}$$ | 无标号 | 有标号 	| 盒非空 | $$\binom{n-1}{r-1}$$ | | $r$ 元 $n$ 次单项式构型数 |
