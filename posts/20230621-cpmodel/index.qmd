...
```c++
// polynomial universal, 2023/06/26
...
```c++
// FNTT & convolution, iteration style, 2023/06/26
vector<ll> rev;
void spawn_rev(ll n){ // n=log2(N)
	rev.resize(1<<n);
	rev[0]=0; for(ll i=1;i<(1<<n);i++) rev[i]=(rev[i>>1]>>1)+((i&1)<<(n-1));
...
给定 $n-1$ 次多项式 $A(x)$，求解满足 $A(x) B(x) = 1 \pmod{x^{n}}$ 的多项式 $B(x)$．

多项式逆元存在的充分必要条件是其常数项非零，若存在则一定唯一．这结论可直接由下述求解方法得到．不失一般性，我们只研究 $A(x)$ 的次数为奇数 $2n-1$ 的情况．设
$$
\begin{aligned}
A(x) &= A_0(x) + x^n A_1(x) \\
B(x) &= B_0(x) + x^n B_1(x)
...
$$
的基础上求解 $A(x)$ 的逆元．边界条件 $b_0 = \frac 1 {a_0}$．时间复杂度均为
$$
