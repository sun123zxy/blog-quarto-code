...
| $${1,0,1}$$ | 有标号 | 无标号 	| 盒非空 | $$\begin{aligned} {n \brace r} &= r {n-1 \brace r} + {n-1 \brace r-1} \\ &= \sum_{k=0}^r \frac{k^n}{k!} \cdot \frac{(-1)^{r-k}}{(r-k)!} \end{aligned}$$ | $$\begin{aligned} \mathrm{EGF}_r(x) &= \frac{(e^x - 1)^r}{r!} \\ \mathrm{OGF}_n(x) &= e^{-x} \sum_{k=0}^{+\infty} k^n \frac{x^k}{k!} \end{aligned}$$ | 第二类 Stirling 数 |
| $${1,\emptyset,\emptyset}$$ | 有标号 | 不限制 	|       | $$\sum_{k=0}^n {n \brace k}$$ | $$\mathrm{EGF}(x)=e^{e^x - 1}$$ | Bell 数；$[n]$ 上等价关系（划分）个数 |
...
第二类 Stirling 数的组合定义即 $1,0,1$-球盒问题的方案数 $\mathrm{BB}_{1,0,1}(n,r) = {n \brace r}$，亦作将 $n$ 元集合划分为 $r$ 个等价类的方案数． 
...
#### 第二类 Striling 数同列计算：$\mathrm{EGF}_r^{1,0,1}(x)$，$\mathrm{EGF}_r^{1,1,0}(x)$ 与 $\mathrm{EGF}^{1,\emptyset,\emptyset}(x)$（Bell 数）
...
\end{aligned}
$$
进一步，注意到 $1,0,1$-球盒问题和 $1,\emptyset,\emptyset$-球盒问题的关系
$$
\mathrm{BB}_{1,\emptyset,\emptyset} = \sum_{r=0}^{+\infty} \mathrm{BB}_{1,0,1}(n,r)
$$
我们有
$$
\mathrm{EGF}^{1,\emptyset,\emptyset}(x) = \sum_{r=0}^{+\infty} \mathrm{EGF}_r^{1,0,1}(x) = \sum_{r=0}^{+\infty} \frac{(e^x - 1)^r}{r!} = e^{e^x - 1}
$$
此即 $1,\emptyset,\emptyset$-球盒问题的方案数的指数生成函数．此即 Bell 数的组合定义．Bell 数的另一种组合意义是 $n$ 元集合上等价关系（划分）的数量．Bell 数也是第二类 Stirling 数的同行求和． 
