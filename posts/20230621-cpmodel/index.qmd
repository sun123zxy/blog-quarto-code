...
|:-:|:-:|:-:|:--:|:--:|:--:|
| 有标号 | 有标号 	| 允许	 | $$r^n$$ | | $[n] \to [r]$ 映射个数 |
| 有标号 | 有标号 	| 不允许 | $$r! {n \brace r}$$ | $$\mathrm{EGF}_n(x)=(e^x - 1)^r$$ | $[n] \to [r]$ 满射个数 |
| 有标号 | 无标号 	| 允许	 | $$\sum_{k=0}^r {n \brace k}$$ | $$\mathrm{EGF}_n(x)=\sum_{k=0}^r \frac {(e^x - 1)^k}{k!}$$ | |
| 有标号 | 无标号 	| 不允许 | $${n \brace r} = r {n-1 \brace r} + {n-1 \brace r-1}$$ | $$\begin{aligned} \mathrm{EGF}_n(x) &= \frac{(e^x - 1)^r}{r!} \\ \mathrm{EGF}_r(x) &= e^{-x} \sum_{k=0}^{+\infty} k^n \frac{x^k}{k!} \end{aligned}$$ | 第二类 Stirling 数 |
| 有标号 | 不限制 	|       | $$\sum_{k=0}^n {n \brace k}$$ | $$\mathrm{EGF}_n(x)=e^{e^x - 1}$$ | Bell 数；$[n]$ 上等价关系（划分）个数 |
| 无标号 | 有标号 	| 允许	 | $$\binom{n+r-1}{r-1}$$ | | $r$ 元不定方程非负整数解个数 |
| 无标号 | 有标号 	| 不允许 | $$\binom{n-1}{r-1}$$ | | $r$ 元 $n$ 次单项式构型数 |
| 无标号 | 无标号 	| 允许	 | $$\sum_{k=0}^r p(n,k)$$ | | |
| 无标号 | 无标号 	| 不允许 | $$p(n,r)=p(n-1,r-1)+p(n-r,r)$$ | | $r$ 部分拆数 |
| 无标号 | 不限制	|       | $$\begin{aligned} p(n) &= \sum_{\substack{k \in \mathbb Z}} (-1)^k p_{n-k(3k-1)/2} \\ &= p_{n-1} + p_{n-2} - p_{n-5} - p_{n-7} + \dots \end{aligned}$$ | $$\begin{aligned} \mathrm{OGF}_n(x)&=\prod_{k=1}^{+\infty} \frac 1 {1-x^k} \\ &= \left( \sum_{k \in \mathbb Z} (-1)^k x^{k(3k-1)/2} \right)^{-1} \end{aligned}$$ | 分拆数 |
:::

### Stirling 数

考虑用容斥非空盒的方法得到球有标号、盒无标号、不允许空盒的球盒问题的解．我们有
$$
...
$$
这就是第二类 Stirling 数的通项公式．注意到其具有卷积的形式（即上文 $\mathrm{EGF}_r(x)$），由此可快速计算出同一行的第二类 Stirling 数．

快速计算同一列，考虑将大小为 $n$ 的有标号集合划分为 $r$ 个无标号等价类．这相当于对有序拼接 $r$ 个非零集合后除掉 $r!$ 消序，写成生成函数的形式就得到上文所述 $\mathrm{EGF}_n(x)$．进一步注意到其具有指数函数幂级数的形式，无穷求和就得到 Bell 数的 $\mathrm{EGF}_n(x)$．
