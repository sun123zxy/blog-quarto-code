...
Correspondence of arbitrary ideals is not mentioned in the statement because its subtlety: $\mathfrak a \mapsto S^{-1} \mathfrak a$ is indeed surjective, but is not injective because in $S^{-1} R$ one can use elements in $S$ to "cancel out" the numerators. See the proof for details.
